...
var path = require('path'); // 'content-type' to 'Content-Type'
...
function isMatchingData(reqURL, reqMethod, item = {}) {
  var {
    url,
    method
  } = Object.assign({}, item, item.request);
  ...
  if (reqURL.toLowerCase() === url.toLowerCase() || reg.test(reqURL)) {
    return true;
  }
...
function searchMatchingData(url, method, dataPath, searchOrder) {
  ...
  if (filenames && filenames.length > 0) {
    // 文件排序
    filenames = searchOrder && searchOrder(filenames) || []; // 遍历所有 mock 数据

    for (let i = 0; i < filenames.length; i++) {
      let filename = filenames[i];
      let filePath = path.join(dataPath, filename);
      let fileStat = fs.statSync(filePath); // 是文件则对比请求与文件中的 mock 数据是否匹配

      if (fileStat.isFile()) {
        let matchingData = getMatchingData(filePath, url, method); // 如果找到了则返回, 未找到继续递归查找.
