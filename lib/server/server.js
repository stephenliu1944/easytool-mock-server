...
function sendResponse(mockResponse, staticPath, res, next) {
  ...
  setTimeout(function () {
    ...
    if (/\.\w{1,5}$/.test(body)) {
      // 发送文件
      res.sendFile(body, {
        root: path.join(process.cwd(), staticPath)
      }, function (err) {
        err && next(err);
      });
    } else {
...
function startup(options = {}, config = 'mock.config.js') {
  var settings = getCustomSettings(path.join(process.cwd(), config));
  ...
  app.use(function (req, res, next) {
    var mockDataPath = path.join(process.cwd(), sourcePath);
    console.log('mockDataPath: ', mockDataPath); // 从 mock data 数据源中找到匹配的数据
